...
@export var slip_needle: Sprite2D
@export var power_line: Line2D
@export var torque_line: Line2D
@export var slip_line: Line2D
...
func _process(_delta: float) -> void:
	...
	if Global.process_frames % Global.Priority.HIGH == 0:
		calculate.speedHud(speed, abs(Global.ChassisRB.wheels_angular_kph[Global.ChassisRB.transmission.power_distribution])) # abs(Global.ChassisRB.wheels_angular_kph[Global.ChassisRB.transmission.power_distribution]) # Global.ChassisRB.kph
		calculate.slip(slip_line, Global.ChassisRB.curve.slip_ratio_curve, abs(Global.ChassisRB.frictions[Global.ChassisRB.transmission.power_distribution]))

	$LAnchor/Label3.text = "fps: %s" % [Engine.get_frames_per_second()]

	if Global.process_frames % Global.Priority.MEDIUM == 0:
		...
		tween2.tween_property(slip_needle, "position", Vector2(Global.ChassisRB.slip_ratios[Global.ChassisRB.transmission.power_distribution] * 100 * 0.5 + slip_line.position.x, slip_needle.position.y), 0.066)

...
func process_drag(chassis:RigidBody2D, speed_kph:float, drag_coef:float, frontal_area:float, air_density:float, gravity:float, magnitude_:float):
	if _process_drag:
		var drag = 0.5 * drag_coef * frontal_area * air_density
		var Fdrag = (-(drag * pow(speed_kph, 2)) * magnitude_) / gravity
		chassis.constant_force.x = Fdrag
	else:
		chassis.constant_force.x = 0

func process_rolling_resistance(wheels:Array[Node], rr_coef:float, speed_kph:float, gravity:float, chassis_weight:float):
	if _process_rolling_resistance:
		var rr = rr_coef * chassis_weight
		var Frr = (-rr * speed_kph) / gravity
		for wheel in wheels:
			wheel.constant_force.x = Frr * 2
	else:
		for wheel in wheels:
...
func process_friction(slip_ratio_curve:Curve, slip_ratio_rear:float, slip_ratio_front, wheels:Array[Node], friction_coef:float):
	wheels[0].physics_material_override.friction = friction_coef * slip_ratio_curve.sample(slip_ratio_rear) #clampf(friction_coef * abs(slip_ratio_curve.sample(slip_ratio_rear)), friction_coef * slip_ratio_curve.get_point_position(3).y, friction_coef)
	wheels[1].physics_material_override.friction = friction_coef * slip_ratio_curve.sample(slip_ratio_front) #clampf(friction_coef * abs(slip_ratio_curve.sample(slip_ratio_front)), friction_coef * slip_ratio_curve.get_point_position(3).y, friction_coef)


func process_brakes(wheels:Array[Node],brake_base:float, brake_peak:float, brake_exponent:float, engine_inertia:float,wheel_rpm:float, redline_rpm:float, rpm_limit:float,brake_peak_rpm:float ,brake:float, handbrake:float, front_brake:float, rear_brake:float, wheel_magnitude_front:float, wheel_magnitude_rear:float, gear_i:float, gears:Array, throttle:float, anti_braking_rear:float, anti_braking_front: float):
	if _process_engine_brake_:
		...
		if gear_i == 1:
			...
			if is_zero_approx(throttle):
				...
				wheels[0].constant_torque = Fbrake * abs(gears[gear_i]) * (engine_inertia * 100)
		# braking
		if handbrake:

...
@export var speed_grind := [10, 666]
@export_group("Dust")
@export var _dust := true
@export var dust_color_power: Curve
@export var speed_dust := [130, 666]

func friction(speed_kph: float, frictionScenes: Array[GPUParticles2D], wheel_magnitude: float, wheel:RigidBody2D,slip_ratio: float):
	if _friction:
		if int(wheel_magnitude) == 0: wheel_magnitude = 1
		if speed_friction[0] < speed_kph and speed_kph < speed_friction[1] and abs(wheel.linear_velocity.x) > 5.0:
...
func slip(speed_kph: float, slipScenes: Array[GPUParticles2D], wheel_magnitude: float, wheel:RigidBody2D,slip_ratio: float):
	if _slip:
		if int(wheel_magnitude) == 0: wheel_magnitude = 1
		if (speed_slip[0] < speed_kph and speed_kph < speed_slip[1] and abs(wheel.linear_velocity.x) > 5.0):
...
func grind(speed_kph: float, grindScenes: Array[GPUParticles2D], wheel_magnitude: float, wheel:RigidBody2D, slip_ratio: float):
	if _grind:
		if int(wheel_magnitude) == 0: wheel_magnitude = 1
		if (speed_grind[0] < speed_kph and speed_kph < speed_grind[1] and abs(wheel.linear_velocity.x) > 5.0):

...
func _process(_delta: float) -> void:
	$Label.text = "Speed: %.fkph %.fmph %.fmps\nAccel: %f\nRPM: %.f\nGear: %.f\nPower: %.fkW\nTorque: %.fNm\nSlip Ratio rear: %f\nFriction rear: %f\nDrag: %f\nRolling Resistance: %f\nEngine Brake: %f" % [kph, kph * 0.621371, mps, acceleration, wheel_rpm, gear_i - 1, curve.power_curve.sample(wheel_rpm), curve.torque_curve.sample(wheel_rpm), slip_ratio_rear, WheelsRB[0].physics_material_override.friction, ChassisRB.constant_force.x, WheelsRB[0].constant_force.x, WheelsRB[0].constant_torque]
...
func _physics_process(delta: float) -> void:
	...
	calculate.process_rolling_resistance(WheelsRB, tire.rolling_resistance, mps, chassis.mass)
	calculate.process_brakes(WheelsRB, engine.engine_brake_base, engine.engine_brake_peak, engine.engine_brake_exponent, wheel_rpm, engine.redline_rpm, 
